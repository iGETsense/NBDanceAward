# 📥 Importer les Candidats dans Firebase

## Méthode 1: Via Admin Panel (Facile) ✅

### Étapes:
1. Démarrez l'application:
   ```bash
   npm run dev
   ```

2. Allez à: `http://localhost:3000/admin`

3. Connectez-vous:
   - **Mot de passe**: `NB2024Admin`

4. Cliquez sur **"Réinitialiser Firebase"** (section rouge)

5. Confirmez l'action

6. La page se rechargera automatiquement avec les 22 candidats

---

## Méthode 2: Via Firebase Console (Manuel)

### Étapes:

1. Allez à: https://console.firebase.google.com

2. Sélectionnez le projet: `project-5583295336911612869`

3. Allez à **Realtime Database**

4. Cliquez sur **"+"** pour ajouter des données

5. Entrez le chemin: `candidates`

6. Collez le JSON complet (voir ci-dessous)

7. Cliquez sur **"Add"**

---

## JSON Complet à Importer

Copier-coller ce JSON dans Firebase Console:

```json
{
  "etienne-kampos": {
    "id": "etienne-kampos",
    "name": "Étienne kampos",
    "category": "Meilleur artiste danseur - masculin",
    "categoryId": "male-dancer",
    "image": "/dancers/Etienne kampos.jpg",
    "votes": 1847,
    "badge": 1,
    "percentage": 45,
    "title": "Male Dance King"
  },
  "de-flow": {
    "id": "de-flow",
    "name": "De Flow",
    "category": "Meilleur artiste danseur - masculin",
    "categoryId": "male-dancer",
    "image": "/dancers/De Flow.jpeg",
    "votes": 1654,
    "badge": null,
    "percentage": 40,
    "title": "Flow Master"
  },
  "pascal-metaphore": {
    "id": "pascal-metaphore",
    "name": "Pascal métaphore",
    "category": "Meilleur artiste danseur - masculin",
    "categoryId": "male-dancer",
    "image": "/dancers/PASCAL métaphore.jpeg",
    "votes": 1432,
    "badge": null,
    "percentage": 35,
    "title": "Poetic Dancer"
  },
  "maguy-merine": {
    "id": "maguy-merine",
    "name": "Maguy merine",
    "category": "Meilleure artiste danseuse féminine",
    "categoryId": "female-dancer",
    "image": "/dancers/MAGUY MERINE.jpeg",
    "votes": 2534,
    "badge": 1,
    "percentage": 62,
    "title": "Dance Star"
  },
  "kendi": {
    "id": "kendi",
    "name": "Kendi",
    "category": "Meilleure artiste danseuse féminine",
    "categoryId": "female-dancer",
    "image": "/dancers/KENDI.jpeg",
    "votes": 2245,
    "badge": 2,
    "percentage": 55,
    "title": "Dance Performer"
  },
  "bebs-velina": {
    "id": "bebs-velina",
    "name": "Beb's velina",
    "category": "Meilleure artiste danseuse féminine",
    "categoryId": "female-dancer",
    "image": "/dancers/bebs-velina.jpeg",
    "votes": 1956,
    "badge": null,
    "percentage": 48,
    "title": "Talented Dancer"
  },
  "afu-dance-academy": {
    "id": "afu-dance-academy",
    "name": "AFU Dance académie",
    "category": "Meilleur groupe de danse",
    "categoryId": "dance-group",
    "image": "/dancers/AFU DANCE ACADEMY STUDIO.jpeg",
    "votes": 2089,
    "badge": 1,
    "percentage": 51,
    "title": "Professional Group"
  },
  "etat-nwar-dance": {
    "id": "etat-nwar-dance",
    "name": "Etat NWAR dance",
    "category": "Meilleur groupe de danse",
    "categoryId": "dance-group",
    "image": "/dancers/ÉTAT NWAR DANCE SCHOOL.jpg",
    "votes": 1834,
    "badge": null,
    "percentage": 45,
    "title": "Dance Collective"
  },
  "team-escram": {
    "id": "team-escram",
    "name": "Team Escram",
    "category": "Meilleur groupe de danse",
    "categoryId": "dance-group",
    "image": "/dancers/TEAM ESCRAM.jpeg",
    "votes": 1689,
    "badge": null,
    "percentage": 41,
    "title": "Dance Crew"
  },
  "deboy-le-monstre": {
    "id": "deboy-le-monstre",
    "name": "Déboy le monstre et Maguy merine",
    "category": "Meilleur collaboration duo",
    "categoryId": "duo-collaboration",
    "image": "/dancers/DEBOY LE MONSTRE.jpeg",
    "votes": 1956,
    "badge": 1,
    "percentage": 48,
    "title": "Monster & Queen"
  },
  "accadien-fureur": {
    "id": "accadien-fureur",
    "name": "Accadien fureur",
    "category": "Meilleur artiste Chorégraphe",
    "categoryId": "choreographer",
    "image": "/dancers/Accadient Fureur.jpeg",
    "votes": 2245,
    "badge": 1,
    "percentage": 55,
    "title": "Arcadian Choreographer"
  },
  "goldy-lastar": {
    "id": "goldy-lastar",
    "name": "Goldy lastar",
    "category": "Meilleur artiste Chorégraphe",
    "categoryId": "choreographer",
    "image": "/dancers/GOLDY LA-STAR.jpeg",
    "votes": 2089,
    "badge": null,
    "percentage": 51,
    "title": "Golden Star"
  },
  "kendi-web": {
    "id": "kendi-web",
    "name": "Kendi",
    "category": "Meilleur Performance web",
    "categoryId": "web-performance",
    "image": "/dancers/KENDI.jpeg",
    "votes": 3245,
    "badge": 1,
    "percentage": 79,
    "title": "Web Star"
  },
  "deboy-web": {
    "id": "deboy-web",
    "name": "Déboy le monstre",
    "category": "Meilleur Performance web",
    "categoryId": "web-performance",
    "image": "/dancers/DEBOY LE MONSTRE.jpeg",
    "votes": 2834,
    "badge": 2,
    "percentage": 69,
    "title": "Viral Sensation"
  },
  "ayi-ventilateur": {
    "id": "ayi-ventilateur",
    "name": "Ayi ventilateur",
    "category": "Meilleur artiste danseur au rythme folklorique",
    "categoryId": "folkloric-dancer",
    "image": "/dancers/ayi ventilateur.png",
    "votes": 1834,
    "badge": 1,
    "percentage": 45,
    "title": "Wind Master"
  },
  "ordinateur-baboue": {
    "id": "ordinateur-baboue",
    "name": "Ordinateur baboué",
    "category": "Meilleur artiste danseur afro coupé décalé",
    "categoryId": "afro-decale-dancer",
    "image": "/dancers/ORDINATEUR baboué.jpeg",
    "votes": 2156,
    "badge": 1,
    "percentage": 53,
    "title": "Afro Master"
  },
  "shazam-le-vrai": {
    "id": "shazam-le-vrai",
    "name": "Shazam le vrai",
    "category": "Meilleur artiste danseur afro coupé décalé",
    "categoryId": "afro-decale-dancer",
    "image": "/dancers/SHAZAM.jpeg",
    "votes": 1923,
    "badge": null,
    "percentage": 47,
    "title": "Décalé Star"
  },
  "petit-tchakap-mbole": {
    "id": "petit-tchakap-mbole",
    "name": "Petit tchakap",
    "category": "Meilleur artiste danseur mbolé",
    "categoryId": "mbole-male-dancer",
    "image": "/dancers/petit tchakap.jpg",
    "votes": 1867,
    "badge": 1,
    "percentage": 46,
    "title": "Mbolé Master"
  },
  "nelly-dora": {
    "id": "nelly-dora",
    "name": "Nelly Dora",
    "category": "Meilleure artiste danseuse mbolé",
    "categoryId": "mbole-female-dancer",
    "image": "/dancers/NELLY DORA.jpeg",
    "votes": 1956,
    "badge": 1,
    "percentage": 48,
    "title": "Mbolé Master"
  },
  "el-fally-year": {
    "id": "el-fally-year",
    "name": "El Fally du 237",
    "category": "Meilleur artiste danseur de l'année",
    "categoryId": "male-dancer-year",
    "image": "/dancers/El fally du 237.jpg",
    "votes": 3456,
    "badge": 1,
    "percentage": 84,
    "title": "Dancer of the Year"
  },
  "maxime-la-vitesse": {
    "id": "maxime-la-vitesse",
    "name": "Maxime la vitesse",
    "category": "Meilleur artiste jeune danseur/danseuse",
    "categoryId": "young-dancer",
    "image": "/dancers/Maxime la vitesse.jpg",
    "votes": 2234,
    "badge": 1,
    "percentage": 54,
    "title": "Young Prodigy"
  },
  "katia-eg-year": {
    "id": "katia-eg-year",
    "name": "Katia eg",
    "category": "Meilleure artiste danseuse de l'année",
    "categoryId": "female-dancer-year",
    "image": "/dancers/Katia EG.png",
    "votes": 3567,
    "badge": 1,
    "percentage": 87,
    "title": "Female Dancer of Year"
  }
}
```

---

## Vérification

Après l'import, vérifiez:

1. ✅ Allez à `http://localhost:3000`
2. ✅ Les candidats s'affichent par catégorie
3. ✅ Pas de "Unknown Category"
4. ✅ Les images se chargent
5. ✅ Les pourcentages s'affichent

---

## Problèmes?

Si les données ne s'affichent toujours pas:

1. Rechargez la page (Ctrl+F5)
2. Videz le cache du navigateur
3. Vérifiez la console (F12) pour les erreurs
4. Réessayez l'import

**Besoin d'aide? Consultez `FIREBASE_FIX.md`**
